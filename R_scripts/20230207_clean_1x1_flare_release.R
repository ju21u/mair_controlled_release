...
for (current.idx in seq(length(segment.inputs))) {
  ...
  write.csv(RF05_release_summary,file.path(out.folder, paste('RF05_1000_',  as.character(current_thres), '.csv', sep = '' )))
    }
  ...
  #### FLARE 
    if(run_flare){
      ...
      write.csv(RF05_flare_summary,file.path(out.folder, paste('RF05_flare_10_',  as.character(current_thres), '.csv', sep = '' )))
    }

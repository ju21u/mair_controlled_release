...
for (current_thres in thres_all){
 ...
 # for (current.idx in c(7)) {
  ...
  write.csv(RF04_release_summary,file.path(out.folder, paste('230223_RF04_1000_',  as.character(current_thres), '.csv', sep = '' )))
...
for (current.idx in seq(length(segment.inputs))) {
  ...
  write.csv(RF05_release_summary,file.path(out.folder, paste('230223_RF05_1000_',  as.character(current_thres), '.csv', sep = '' )))

...
source("/n/holylfs04/LABS/wofsy_lab/Users/achulakadabba/R_scripts/MethaneAIR/unique_scripts/estimate_emissions_v8.R")
...
for (current_thres in thres_all){
  ...
  RF04_release_summary<- data.frame(matrix(vector(), 0, 16,
 ...
 # RF08.sources = cbind(raw_maryann_data$x_min, raw_maryann_data$x_max, raw_maryann_data$y_min, raw_maryann_data$y_max)
  ...
  inflow_sides[inflow_sides > -135  & inflow_sides < -45] = 4 # WEST
  inflow_sides[inflow_sides > 45  & inflow_sides < 135] = 3 # SOUTH
  inflow_sides[inflow_sides > 5 | inflow_sides < -5] = 2 # EAST
  
  RF08.sources.inflow = cbind(raw_maryann_data$x_min, raw_maryann_data$x_max, raw_maryann_data$y_min, raw_maryann_data$y_max)
  RF08.sources.inflow[inflow_sides == 1, 4] = RF08.sources.inflow[inflow_sides == 1, 3] + 0.01
  RF08.sources.inflow[inflow_sides == 2, 2] = RF08.sources.inflow[inflow_sides == 2, 1] + 0.01
  RF08.sources.inflow[inflow_sides == 3, 3] = RF08.sources.inflow[inflow_sides == 3, 4] - 0.01
  RF08.sources.inflow[inflow_sides == 4, 1] = RF08.sources.inflow[inflow_sides == 4, 2] - 0.01

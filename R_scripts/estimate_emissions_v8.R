estimate_emissions = function( current.research.flight = "RF04", 
  ...
  #  setwd(current.path)
   ...
   # browser()
    ...
    SOURCE_INFLOW_MEDIAN = median(raster::values(source_inflow)) 
    SOURCE_INFLOW_SD =  sd(raster::values(source_inflow)) 
   ...
   # plot(entire_raster_input, col = heat.275, main = "DI & mIME extents")
 ...
 #   plot(source_raster, col = heat.275, main = "thres only")
  ...
  #browser()
    ...
    skipping = TRUE
    if (exists("source_time")){
    YYYYMMDD = str_replace_all(current.date, "-", "")
    seconds_since_midnight = floor((source_time$start[current.segment.input] + source_time$stop[current.segment.input])/2 * 60 * 60)
    export_name = paste(YYYYMMDD, seconds_since_midnight, format(raw_maryann_data$x_cent[current.segment.input], nsmall = 6), format(raw_maryann_data$y_cent[current.segment.input], nsmall = 6), "RF08_uinta_mIME.nc", sep = "_")
    print(export_name)}
    #browser()
    ...
    if(is.null(skip_winds)){
      date = rep(current.date, each = length(current.segment.input))
      t.start = as.numeric(summary.current.RF$start[is.element(summary.current.RF$seg, current.segment.input)])
      t.stop = as.numeric(summary.current.RF$stop[is.element(summary.current.RF$seg, current.segment.input)])
...
#    plot(resample_inflow, col = heat.275, main = "resample inflow")
    ...
    resample.ime.emiss = emission.cal.IME(wind = current.wind, ime_input = resample_kg, area_input = current.area)
...
#browser()
  return(list(df_summary_output[1, ],  cbind(les.mass.all, as.vector(current.winds), export_name) ))}
    else{
      return(list(df_summary_combined[1, ],  cbind(NULL, NULL, export_name) ))
    }
}
